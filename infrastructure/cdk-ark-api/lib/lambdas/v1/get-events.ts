...
export function getEventsLambda(
  scope: cdk.Stack,
  vpc: IVpc,
  ...
  const getContractLambda = new RustFunction(scope, "get-events", {
  ...
  for (const stage of stages) {
    ...
    resourceArns.push(
      `arn:aws:dynamodb:${scope.region}:${scope.account}:table/${tableNamePrefix}_${stage}/index/GSI6PK-GSI6SK-index`
    );
  }
