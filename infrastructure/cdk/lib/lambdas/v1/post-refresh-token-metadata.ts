...
export function postRefreshTokenMetadataLambda(scope: cdk.Stack, stages: string[]) {
  const postRefreshTokenMetadataLambda = new RustFunction(scope, "post-refresh-token-metadata", {
    manifestPath: '../../ark-lambdas/apigw/lambda-post-refresh-token-metadata/Cargo.toml',
    environment: {
      RUST_BACKTRACE: "1",
    },
