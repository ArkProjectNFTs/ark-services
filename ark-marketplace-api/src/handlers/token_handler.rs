...
use crate::db::query::{get_collection_floor_price, get_token_data, get_token_offers_data, get_tokens_data, get_tokens_portfolio_data};
...
pub async fn get_token_offers<D: DatabaseAccess + Sync>(
    ...
    let db_access = db_pool.get_ref();
    let floor_price = match get_collection_floor_price(db_access, &normalized_address, &chain_id).await {
    ...
    let token_offers_data = match get_token_offers_data(
        db_access, &normalized_address, &chain_id, &token_id
    ).await{
        Err(sqlx::Error::RowNotFound) => return HttpResponse::NotFound().body("data not found"),
    ...
    let token_offers_data: Vec<TokenOfferOneData> = token_offers_data.iter().map(|data| TokenOfferOneData {
        offer_id: data.offer_id,
        price: data.amount.clone(),
        source: data.source.clone(),
        expire_at: data.expire_at,
        hash: data.hash.clone(),
        floor_difference: compute_floor_difference(data.amount.clone(), data.currency_address.clone(), floor_price.value.clone())
    }).collect();
